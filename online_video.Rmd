...
The statistical model corresponds to our initial observations when exploring the data: the probability of watching online video decreases with age and increases with educational level and income. Using the model, we can now quantify the impact of variation in each predictor on the outcome. This final section demonstrates how this can be used in planning direct services using online video.

As an example, a direct service provider may decide that its decision about whether to invest in online video should be based on whether its service users are likely to engage with communications via this channel. They may want to use a threshold for committing resources, such as that 50% of a segment watches online video. We can use the logistic regression model to compute estimate this probability based on known demographic characteristics of the service users.

For this example, let's assume that the direct service provider is working with a popualtion that is predominantly working-age and low-incomew. On the one hand, the negative coefficient for age may suggest that online video is appropriate for a working age audience; on the other hand, the positive coefficients for income (and educational level) may suggest caution in investing in online video. How to balance these two competing considerations? 

Using the statistical model, we can compute estimated probabilities for a hypothetical population of adults corresponding to the characteristics of the service users. In this example, we will construct a population that is all non-college educated, split into 5 subgroups with the incomes corresponding to the bottom five income deciles, and with a range of ages within each subgroup from 18 to 80. Plot 4 below shows how the modelled probability of watching online video declines with age for 5 bottom income deciles. 
...
Using this plot, we can predict that the threshold value of 50% is exceeded for all of the bottom five income deciles up until around age 40 years. The curve then declines such that by around age 60 years the probability has decreased to 25% for all of the bottom five income deciles. In this way, our expectations for reaching different segments of the user-base can be quantitatively defined as part of the service planning process. 
